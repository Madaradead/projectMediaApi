...
const isValidTextContent = (content: string): boolean => {
    ...
    if (controlRatio > 0.02) return false;
...
export const updateMetadata = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (parsedBody.data.description !== undefined) updateData.description = parsedBody.data.description;
        if (parsedBody.data.visibility !== undefined) updateData.visibility = parsedBody.data.visibility;
